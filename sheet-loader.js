...
//   confidence | evaluationType | studyDesign | services
//
// The last three columns (evaluationType, studyDesign, services) are
// OPTIONAL in the sheet. If your sheet doesn't have them yet, or leaves a
...
// project's evaluationType (see deriveServicesFallback below).
//
// For columns that hold MULTIPLE values (theme, subtheme, evaluationType,
// studyDesign, services), separate values with a comma inside the cell, e.g.:
//   Education, Health
//
// Expected values —
...
const LIST_FIELDS = ["theme", "subtheme", "evaluationType", "studyDesign", "services"];
...
function rowsToProjects(rows) {
  ...
  return dataRows.map((cells, idx) => {
    ...
    // Services: sheet value wins; otherwise derive a starting point from
    // evaluationType (see deriveServicesFallback above).
