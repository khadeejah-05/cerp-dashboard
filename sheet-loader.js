...
// sheet's value wins over the fallback automatically. "services" also has
// a second fallback layer: if neither the sheet nor MANUAL_TAG_OVERRIDES
// has a value for a project, it's derived automatically from that
// project's evaluationType (see deriveServicesFallback below). "verticals"
// has no fallback — it's a new tagging dimension, so leave it blank until
// each project has been reviewed and tagged by hand.
//
...
//                    "Pre-Post / Before-After (no comparison group)",
//                    "Cross-Sectional", "Longitudinal", "Qualitative"
//   services:        "Monitoring only", "Evaluation only",
//                     "Process evaluation", "Impact evaluation"
//                     (sub-types of "Evaluation only"),
//                     "Monitoring and evaluations",
//                     "MLE (Monitoring, Learning, and Evaluations)"
...
const MANUAL_TAG_OVERRIDES = {
  ...
  "edu-happiness": { evaluationType: ["Impact Evaluation"], studyDesign: ["Pre-Post / Before-After (no comparison group)"] },
  ...
  "gov-imam-outreach": { evaluationType: ["Impact Evaluation"], studyDesign: ["Pre-Post / Before-After (no comparison group)"] },
  ...
  "gov-procurement-efficiency": { evaluationType: ["Impact Evaluation"], studyDesign: ["Pre-Post / Before-After (no comparison group)"] },
  ...
  "health-micare-bep-business-model": { evaluationType: ["Impact Evaluation"], studyDesign: ["Pre-Post / Before-After (no comparison group)"] },
...
// ---------------------------------------------------------------------
// Services fallback derivation. The "services" tagging dimension is new
// and the sheet/MANUAL_TAG_OVERRIDES tables don't have it populated yet
// for most projects. As a starting point, derive a reasonable Services
// value from a project's existing evaluationType tags:
//   - "Monitoring, Evaluation, and Learning (MLE)" present -> MLE
//   - both Process + Impact Evaluation present -> Evaluation only, with
//     both Process evaluation and Impact evaluation sub-tags
//   - only one of Process/Impact Evaluation present -> Evaluation only,
//     with that one sub-tag
//   - no evaluationType at all -> leave Services blank (we genuinely
//     don't know whether it was monitoring, evaluation, both, or neither)
// This is only a starting point — once real Services values are added to
// the sheet for a project, the sheet's value wins automatically and this
// derivation is skipped for that row.
// ---------------------------------------------------------------------
function deriveServicesFallback(evaluationType) {
  if (!evaluationType || !evaluationType.length) return [];
  const hasMLE = evaluationType.includes("Monitoring, Evaluation, and Learning (MLE)");
  const hasProcess = evaluationType.includes("Process Evaluation");
  const hasImpact = evaluationType.includes("Impact Evaluation");

  if (hasMLE) return ["MLE (Monitoring, Learning, and Evaluations)"];

  const out = [];
  if (hasProcess || hasImpact) {
    out.push("Evaluation only");
    if (hasProcess) out.push("Process evaluation");
    if (hasImpact) out.push("Impact evaluation");
  }
  return out;
}
...
function rowsToProjects(rows) {
  ...
  return dataRows.map((cells, idx) => {
    ...
    const project = {
      ...
      // Map your sheet columns to the dashboard fields:
      theme: splitList(obj.theme),
      subtheme: splitList(obj.subtheme),
      
      // Map evaluationType from your sheet (if you have it)
      // If not, we'll derive it from tier2/serviceLine
      evaluationType: obj.evaluationType ? splitList(obj.evaluationType) : [],
      
      // Map studyDesign from tier3 column
      studyDesign: splitList(obj.tier3 || ""),
      
      // Map services from serviceLine column
      services: splitList(obj.serviceLine || ""),
      
      // Map verticals from tier2 column
      verticals: splitList(obj.tier2 || "")
    };

    // Belt-and-braces: ensure every list field is a real array
    LIST_FIELDS.forEach(field => {
      if (!Array.isArray(project[field])) project[field] = [];
    });

    // If evaluationType is empty, try to derive it from tier2/serviceLine
    if (!project.evaluationType.length) {
      // Check if tier2 contains evaluation-related terms
      const tier2 = obj.tier2 || "";
      const serviceLine = obj.serviceLine || "";
      
      if (serviceLine.includes("Evaluations") || tier2.includes("Impact") || tier2.includes("Process")) {
        if (tier2.includes("Impact")) project.evaluationType.push("Impact Evaluation");
        if (tier2.includes("Process")) project.evaluationType.push("Process Evaluation");
        if (!project.evaluationType.length && serviceLine.includes("Evaluations")) {
          project.evaluationType.push("Impact Evaluation");
        }
      }
    }

    // Fall back to manual overrides if available
    ...
    // Derive services from evaluationType if still empty
    if (!project.services.length) {
      project.services = deriveServicesFallback(project.evaluationType);
    }
